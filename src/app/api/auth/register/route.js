...
export async function POST(request) {
  const limited = checkRateLimit(rateLimitKey(request, 'register'), { limit: 5 });
  if (!limited.ok) {
    return jsonNoStore(
  ...
  const result = await createUserAccount(body.email, body.password);
  if (!result.ok) {
    return jsonNoStore({ error: result.error }, { status: result.status || 400 });
  }
